        ...
        function obj = wheel2AFC(inStruct,varargin)
            ...
            if strcmp(p.Results.inputType,'cell')
                ...
                if p.Results.alignProbe2Day && strcmp(p.Results.multidayType,'matByTrial')
                    ...
                    probe.probe = mean(probe.probeData(:,1:2),2); probe.probeAR = mean(probe.probeData(:,3:4),2); 
                    probe.probeBias = probe.probeData(:,5);
                    probe.reinfBef=mean(probe.reinfBefData(:,1:2),2);probe.reinfAft=mean(probe.reinfAftData(:,1:2),2);
                    probe.reinfBefAR=mean(probe.reinfBefData(:,3:4),2);probe.reinfAftAR=mean(probe.reinfAftData(:,3:4),2);
                    probe.reinfBefBias = probe.reinfBefData(:,5);probe.reinfAftBias = probe.reinfAftData(:,5);
